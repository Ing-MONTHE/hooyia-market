...
def creer_user_actif(
    email="actif@hooyia.com", username="actif", password="Pass123!", **kwargs
):
...
class CustomUserModelTest(TestCase):
    ...
    def test_email_obligatoire(self):
        with self.assertRaises(ValueError):
            CustomUser.objects.create_user(
                email="", username="nomail", password="Pass!"
            )
    ...
    def test_username_obligatoire(self):
        with self.assertRaises(ValueError):
            CustomUser.objects.create_user(
                email="x@x.com", username="", password="Pass!"
            )
...
class AdresseLivraisonTest(TestCase):
    ...
    def test_une_seule_adresse_par_defaut(self):
        ...
        adresse2 = self._creer_adresse(
            ville="Douala", region="Littoral", is_default=True
        )
...
class ProfilAPITest(APITestCase):
    ...
    def test_modifier_profil_patch(self):
        response = self.client.patch(
            self.url, {"nom": "Mbarga", "prenom": "Alain"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
