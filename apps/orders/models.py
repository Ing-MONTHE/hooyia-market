...
class Commande(models.Model):
    ...
    adresse_livraison_nom = models.CharField(
        max_length=150, verbose_name=_("Nom destinataire")
    )
    adresse_livraison_telephone = models.CharField(
        max_length=20, verbose_name=_("Téléphone livraison")
    )
    adresse_livraison_adresse = models.CharField(
        max_length=255, verbose_name=_("Adresse")
    )
    adresse_livraison_ville = models.CharField(max_length=100, verbose_name=_("Ville"))
    adresse_livraison_region = models.CharField(
        max_length=100, verbose_name=_("Région")
    )
    adresse_livraison_pays = models.CharField(
        max_length=100, default="Cameroun", verbose_name=_("Pays")
    )
    ...
    note_client = models.TextField(
        blank=True, verbose_name=_("Note / Instructions de livraison")
    )

    date_creation = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Date de commande")
    )
    date_modification = models.DateTimeField(
        auto_now=True, verbose_name=_("Dernière mise à jour")
    )
    date_livraison = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Date de livraison")
    )
...
class LigneCommande(models.Model):
    ...
    produit_nom = models.CharField(
        max_length=255, verbose_name=_("Nom du produit (au moment de la commande)")
    )

    quantite = models.PositiveIntegerField(
        validators=[MinValueValidator(1)], verbose_name=_("Quantité")
    )
...
class Paiement(models.Model):
    ...
    mode = models.CharField(
        max_length=20, choices=ModePaiement.choices, verbose_name=_("Mode de paiement")
    )
    ...
    telephone_paiement = models.CharField(
        max_length=20, blank=True, verbose_name=_("Numéro Mobile Money")
    )

    # Référence de la transaction chez PayUnit
    reference_externe = models.CharField(
        max_length=200, blank=True, verbose_name=_("Référence PayUnit")
    )

    # URL de paiement PayUnit vers laquelle rediriger le client
    authorization_url = models.URLField(
        max_length=500, blank=True, verbose_name=_("URL de paiement PayUnit")
    )
    ...
    date_paiement = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Date du paiement confirmé")
    )
