...
class PayUnitWebhookView(APIView):
    ...
    def post(self, request):
        ...
        if not checkout_id:
            ...
            return Response({'erreur': 'checkout_id manquant.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if statut_payunit in ('FAILED', 'CANCELLED'):
            ...
            logger.warning(f"[Webhook] Paiement échoué ({statut_payunit}) | checkout_id: {checkout_id}")
