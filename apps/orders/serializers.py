...
class CommandeDetailSerializer(serializers.ModelSerializer):
    ...
    def get_mode_paiement(self, obj):
        try:
            return (
                obj.paiement.get_mode_paiement_display()
                or obj.paiement.mode_paiement
                or "—"
            )
        except Exception:
            return "—"
...
class CreerCommandeSerializer(serializers.Serializer):
    ...
    adresse_livraison_pays = serializers.CharField(
        required=False, max_length=100, default="Cameroun"
    )
    ...
    def validate(self, data):
        ...
        if not adresse_id:
            ...
            manquants = []
            if not nom:
                manquants.append("adresse_livraison_nom")
            if not telephone:
                manquants.append("adresse_livraison_telephone")
            if not adresse:
                manquants.append("adresse_livraison_adresse")
            if not ville:
                manquants.append("adresse_livraison_ville")
            if not region:
                manquants.append("adresse_livraison_region")
            if manquants:
                raise serializers.ValidationError(
                    _("Champs obligatoires manquants : %(champs)s")
                    % {"champs": ", ".join(manquants)}
                )
        return data
