...
class ProduitViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated and (
            getattr(user, "is_admin", False) or user.is_staff
        ):
            # Admin voit tout
            return (
                Produit.objects.all()
        ...
        if user.is_authenticated and getattr(user, "is_vendeur", False):
            ...
            return (
                Produit.objects.filter(Q(statut="actif") | Q(vendeur=user))
                .distinct()
                .select_related("categorie", "vendeur")
                .prefetch_related("images", "mouvements_stock")
            )
    ...
    def ajouter_image(self, request, pk=None):
        ...
        if produit.vendeur != request.user and not (
            request.user.is_admin or request.user.is_staff
        ):
            return Response(
                {"erreur": _("Permission refusée.")}, status=status.HTTP_403_FORBIDDEN
            )
    ...
    def gerer_stock(self, request, pk=None):
        ...
        if not (
            ...
            or produit.vendeur == request.user
        ):
            return Response(
                {"erreur": _("Permission refusée.")}, status=status.HTTP_403_FORBIDDEN
            )
...
class StatsOverviewView(APIView):
    ...
    def _get_data(self, request):
        ...
        nouveaux_users = CustomUser.objects.filter(
        ...
        commandes_recentes = []
        for c in Commande.objects.select_related("client").order_by("-date_creation")[
            :5
        ]:
            try:
                client_nom = (
                    (c.client.get_full_name() or c.client.username) if c.client else "—"
                )
            except Exception:
                client_nom = "—"
            commandes_recentes.append(
                {
                    "id": c.id,
                    "reference_courte": (
                        str(c.reference)[:8].upper()
                        if c.reference
                        else str(c.id).zfill(6)
                    ),
        ...
        # ── Stocks faibles (6) ──
        stocks_faibles = []
        for p in Produit.objects.filter(stock__lte=F("stock_minimum")).order_by(
            "stock"
        )[:6]:
        ...
        activite_recente = []
        try:
            for log in AuditLog.objects.select_related("utilisateur").order_by("-date")[
                :6
            ]:
        ...
        try:
            paniers_actifs = (
                Panier.objects.filter(items__isnull=False).distinct().count()
            )
        except Exception:
            paniers_actifs = 0
