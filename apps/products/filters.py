...
class ProduitFilter(django_filters.FilterSet):
    ...
    note_min = django_filters.NumberFilter(
        field_name="note_moyenne", lookup_expr="gte", label="Note minimum"
    )
    ...
    slug = django_filters.CharFilter(
        field_name="slug", lookup_expr="exact", label="Slug produit"
    )
    ...
    categorie_slug = django_filters.CharFilter(
        method="filter_categorie_slug", label="Slug catégorie"
    )
    ...
    en_stock = django_filters.BooleanFilter(
        field_name="stock", method="filter_en_stock", label="En stock uniquement"
    )

    # Filtre produits en vedette
    # ?en_vedette=true
    en_vedette = django_filters.BooleanFilter(
        field_name="en_vedette", label="En vedette"
    )

    # Filtre par vendeur
    # ?vendeur=3
    vendeur = django_filters.NumberFilter(
        field_name="vendeur__id", label="Vendeur (ID)"
    )
    ...
    stock_faible = django_filters.BooleanFilter(
        method="filter_stock_faible", label="Stock faible"
    )

    # Filtre stock OK (stock > stock_minimum)
    # ?stock_ok=true
    stock_ok = django_filters.BooleanFilter(method="filter_stock_ok", label="Stock OK")
    ...
    class Meta:
        model = Produit
        fields = ["statut", "en_vedette", "categorie", "vendeur"]
