...
def creer_user(
    username="testuser", email="test@test.com", password="testpass123", **kwargs
):
    """Crée un utilisateur actif. is_active=True obligatoire pour JWT."""
    return User.objects.create_user(
        username=username, email=email, password=password, is_active=True, **kwargs
    )
...
class AvisModelTest(TestCase):
    ...
    def test_str_avis(self):
        ...
        avis = Avis.objects.create(utilisateur=self.user, produit=self.produit, note=5)
...
class AvisAPITest(APITestCase):
    ...
    def test_double_avis_refuse(self):
        ...
        Avis.objects.create(utilisateur=self.user, produit=self.produit, note=3)
        response = self.client.post(
            "/api/avis/", {"produit": self.produit.id, "note": 5}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_avis_non_authentifie_refuse(self):
        ...
        self.client.credentials()
        response = self.client.post(
            "/api/avis/", {"produit": self.produit.id, "note": 4}
        )
