...
urlpatterns = [
    # ── Conversations ────────────────────────────────────────────
    path("", api_views.ConversationListeAPIView.as_view(), name="chat-liste"),
    path("creer/", api_views.ConversationCreerAPIView.as_view(), name="chat-creer"),
    # ── Détail conversation ──────────────────────────────────────
    path(
        "<int:pk>/", api_views.ConversationDetailAPIView.as_view(), name="chat-detail"
    ),
    ...
    path(
        "<int:pk>/upload/",  # ← NOUVEAU
        api_views.UploadFichierAPIView.as_view(),
        name="chat-upload",
    ),
    path(
        "<int:pk>/fichiers/",  # ← NOUVEAU
