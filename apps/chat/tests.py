...
class ChatAPITest(APITestCase):
    ...
    def test_creer_conversation_avec_soi_meme(self):
        ...
        response = self.client.post(
            "/api/chat/creer/", {"utilisateur_id": self.alice.id}
        )
    ...
    def test_detail_conversation_participant(self):
        ...
        MessageChat.objects.create(
            conversation=conv, expediteur=self.alice, contenu="Bonjour"
        )
        MessageChat.objects.create(
            conversation=conv, expediteur=self.bob, contenu="Salut"
        )
    ...
    def test_detail_conversation_non_participant(self):
        ...
        conv = Conversation.objects.create(
            participant1=self.bob, participant2=self.carol
        )
        response = self.client.get(f"/api/chat/{conv.id}/")
    ...
    def test_envoyer_message(self):
        ...
        self.assertTrue(
            MessageChat.objects.filter(
                conversation=conv, contenu="Hello Bob !"
            ).exists()
        )
    ...
    def test_envoyer_message_non_participant(self):
        ...
        response = self.client.post(
            f"/api/chat/{conv.id}/envoyer/", {"message": "intrusion"}
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class ChatWebSocketTest(TransactionTestCase):
    ...
    def setUp(self):
        self.alice = User.objects.create_user(
            username="alice_ws",
            email="alice_ws@test.com",
            password="pass",
            is_active=True,
        )
        self.bob = User.objects.create_user(
            username="bob_ws",
            email="bob_ws@test.com",
            password="pass",
            is_active=True,
        )
        self.conv = Conversation.objects.create(
